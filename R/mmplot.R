...
# will be used.
	...
	if(is.factor(response)){
		plot(u,y,xlab=label,ylab=colnames(object$model[1]),type='n',yaxt='n',...)
		axis(2,0:1,labels=levels(response))
	} else plot(u,y,xlab=label,ylab=colnames(object$model[1]),type='n',...)
	points(u,yplot)
	...
	if(is.numeric(locfit.control$y$nn) || is.null(locfit.control$y$nn)) {
		...
		if(optimres$convergence!=0) {
			...
			if(optimres$par==.7) warning(paste("optim did not move from initial position with nearest neighbor for yhat vs. ", label, ". Plot may be unreliable."))
			nnopt<-optimres$par
		}
	} 
	lp.par<-locfit.control$y
	...
	lp.par$x<-u
	lp.u<-do.call(lp,lp.par)
	locfit.y <- locfit(y ~ lp.u,family=fam,link=link)
	...
	lp.par$x<-u
	lp.u<-do.call(lp,lp.par)
	locfit.yhat <-locfit(yhat~lp.u,family=fam,link=link)
